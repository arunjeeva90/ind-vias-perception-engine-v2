...
OUTPUT_DIR = "outputs/mobile_phone_detector"
...
def draw(image, boxes, scores, classes):
    """
    Draw and print only COCO 'cell phone' detections.
    ...
    for box, score, cl in zip(boxes, scores, classes):
        ...
        print(f"{PHONE_CLASS_NAME} @ ({top} {left} {right} {bottom}) {conf:.3f}")

        cv2.rectangle(image, (top, left), (right, bottom), (0, 255, 255), 2)
        cv2.putText(
            image,
            f"{PHONE_CLASS_NAME} {conf:.2f}",
            (top, max(left - 6, 15)),
